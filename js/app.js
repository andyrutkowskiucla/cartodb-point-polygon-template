(function(){
    ...
    var userName = 'zerocool'
      , polygonTable = 'us_states'
      , polygonTableActiveCols = 'state_name, pop2010'
      , pointTable = 'us_cities_100k'
      , pointTableActiveCols = 'name, state'
      , defaultZoom = 4
      , markerWidth = 5
    ...
    // so maybe someone could write in a statement to detect that
    ...
    var popup = new L.CartoDBPopup()
      ...
      // Hover point style
      , hover_style = {radius:markerWidth, color:"#333", weight:2, opacity:1, fillColor: "#FFCC00", fillOpacity:1, clickable:false}
      // Create the CartoDB layer
    ...
    $('.pfx_radioBox input').click(function(){
      ...
      if (displayVar != 'all'){
        ptTable.setQuery("SELECT cartodb_id," + pointTableActiveCols + ",the_geom_webmercator,ST_ASGEOJSON(the_geom) as geometry FROM " + pointTable + " WHERE cartodb_id > '" + displayVar + "' ");
      }else{
